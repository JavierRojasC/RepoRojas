...
\begin{equation}
\mathbf{B}=\mathbf{Z}_{T}\mathbf{Z}
\label{eq:Burt}
\end{equation}
...
\begin{equation}
T^2=n (\mu_{k}-\mu_{0})^{T}\mathbf{\Sigma_{0}^{-1}}(\mu_{k}-\mu_{0})
\label{eq:T2}
\end{equation}
...
donde $p$ es el número de dimensiones y $\alpha$ es la significancia predeterminada, inicialmente se considera 0.0027.
